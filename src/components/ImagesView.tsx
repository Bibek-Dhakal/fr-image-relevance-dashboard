...
export default function ImagesView() {
    ...
    return (
        <div>
                    ...
                    <div key={img.id}
                        ...
                        <div className="h-48 bg-gray-100 flex items-center justify-center overflow-hidden">
                            <img
                                ...
                                onError={(e) => {
                                    (e.target as HTMLImageElement).src = 'https://placehold.co/400x300?text=Failed+to+Load';
                                }}
                            />
                        </div>
                        <div className="p-4 flex-1 flex flex-col">
                            ...
                            <p className="text-sm text-gray-500 mb-2 capitalize truncate">
                            ...
                            {img.attributes && img.attributes.length > 0 && (
                                <div className="mt-auto flex flex-wrap gap-1">
                                    {img.attributes.slice(0, 3).map((attr, idx) => (
                                        <span key={idx}
                                              className="text-[10px] sm:text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded truncate max-w-[120px]">
                                            {attr}
                                        </span>
                                    ))}
                                    {img.attributes.length > 3 && (
                                        <span
                                            className="text-[10px] sm:text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded">
                                            +{img.attributes.length - 3}
                                        </span>
                                    )}
                                </div>
            ...
            {images.length === 0 && (
                <div
                    className="text-center py-20 text-gray-500 bg-white rounded-xl border border-dashed border-gray-300">
